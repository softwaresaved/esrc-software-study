...
```{r funding_ByOrg, echo = FALSE, fig.alt = "Expenditure across all UKRI councils from the available data."}
...
```{r funding_ByOrg_active, echo = FALSE, fig.alt = "Expenditure across all UKRI councils for active projects from the available data."}
...
A word cloud made from the award titles (some of the longer words are not included).
...
esrcdat %>% filter(Status == "Active")                            %>%
            ...
            with(suppressWarnings(wordcloud(word, n, colours = pal, ordered.colors = TRUE))) 
```
