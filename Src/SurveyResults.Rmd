...
GraphColours <- c("#264f71", "#37718e", "#d5d3c5", "#c9ced6")
...
data %>% select(URN, num_range("Q2_", 1:2))                    %>%
         ...
         group_by(datause)                                     %>% 
         mutate(n = n())                                       %>%  
         ggplot(aes(x = datause, fill = factor(n))) +
         geom_bar(colour = LineColour) +
         theme_minimal() + 
         theme(line = element_line(color = LineColour), legend.position = "None") +
         xlab(my$lab) + ylab("Numbers") + 
...
Data segmented by career stage.
...
data %>% select(num_range("Q2_", 1:2), career = Q20)        %>%
         ...
         geom_text(aes(x = datause, label = after_stat(count)),
...
data %>% select(num_range("Q2_", 1:2), institution = CleanLocs)              %>%
         ...
         geom_text(aes(label = after_stat(count)), position = position_stack(),  
...
### Response distribution and ESRC funding
...
data %>% select(URN, num_range("Q3_", 1:7), esrcfunding)                          %>%
         ...
         theme_bw() + 
         theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0, size = 12)) +
         xlab(my$lab) + ylab("Numbers") + ylim(0, 56) +
         geom_text(aes(label = paste0(n, "\n(", per,")")),position = position_dodge2(width = 0.9, preserve = "single"), 
                   vjust = -0.25, size = 3) +
         scale_fill_manual(name = "ESRC funding", labels = c("no-ESRC funding","ESRC funding", "Unknown"), values = c("red", "green","blue"))
...
Look at the processed data which only takes the last value (most liberal sharing of software) option.
...
funding %>% pivot_longer(cols = num_range("Q18_", 1:7),
            ...
            geom_col(colour = "black") + ylim(0, 120) +
