...
data %>%  select(rew = "Q12")                      %>%
          ...
          ggplot(aes(x = reorder(rew, -n), y = n)) + 
          geom_col(fill = "light blue", colour = "black") +
          theme_bw() +
...
data %>% select(starts_with("Q15_"), -"Q15_a")                          %>%
         ...
         group_by(aware)                                                %>%
         mutate(n = n())                                                %>% 
         mutate(per = percent(n/N, accuaracy = 1))                      %>% 
         ggplot(aes(x = aware)) +
         geom_bar(fill = "light blue", colour = "black") +
         theme_bw() + ylim(0,76) +
...
data %>%  select(num_range("Q16_", 1:13))         %>%
  ...
  geom_text(aes(label = ..count..), stat = "count", vjust = -0.5, size = 3) +
  theme(axis.text.x = element_text(angle = -60, vjust = 0.5, hjust=0, size = 6))
