...
### Response distribution indicating ESRC funding
...
data %>% select(URN, num_range("Q3_", 1:7), esrcfunding)                          %>%
         ...
         geom_col(colour = "black", position = position_stack(vjust = 0.5)) + 
         theme_bw() + 
         theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0, size = 12)) +
         xlab(my$lab) + ylab("Numbers") + ylim(0,83) +
         geom_text(aes(label = paste0(n, " (", per,")")), position = position_stack(vjust = 0.5), size = 4) +
         labs(fill = "ESRC funding") 
```
### Choice correlations
