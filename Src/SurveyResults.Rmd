...
### Use of cata by gender

```{r use_of_data_gender}
...
data %>% select(num_range("Q2_", 1:2), institution = CleanLocs)              %>%
         ...
         theme_bw() +
         geom_point(alpha = 0.25) + xlab("Data use") + ylab("Institutions") +
...
data %>% select(num_range("Q2_", 1:2), institution = CleanLocs)              %>%
         ...
         geom_bar(colour = "black") +
         theme_bw() +
         ylab("Numbers") + xlab("Instituion") + labs(fill = "Data use") +
...
contrbute an A_B_D then we count all the combined choices. Here we only show
where the count > 5.
...
data %>% select(ethnicity = Q22, Institution = CleanLocs) %>%
         ...
         geom_text(aes(x = ethnicity, y = Institution, label = ..count..), na.rm = TRUE,
                   vjust = 1, size = 2, colour = "white") +
                  theme(axis.text.x = element_text(angle = -90, vjust = 0.5, hjust=0)) 
