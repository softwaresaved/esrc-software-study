...
data %>% select(num_range("Q3_", 1:7))                                %>%
         ...
         ggplot(aes(x = reorder(DataSharing, -n), y = n, fill = DataSharing)) +
         geom_col(fill = "light blue", colour = "black") + 
         theme_bw() +
         theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0)) +
         xlab("Data sharing habits") + ylab("Numbers") + ylim(0,83) +
         geom_text(aes(label = paste0(n, " (", per,")")), vjust = -0.5, size = 3) +
         theme(legend.position = "none") 
```
...
important_software %>% select(starts_with("Q5"))             %>%
                       ...
                       ylab("Numbers") + xlab("Most important software") + ylim(0,57) +
...
data %>% select(os = Q6, career = "Q20")           %>%
         ...
         ggplot(aes(x = os, y = n/N, fill = career)) + 
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"), colour = "black") + 
         theme_bw() +
...
data %>% select(starts_with("Q6_a"))              %>%
         ...
         mutate(n = n())                          %>% 
         mutate(per = percent(n/N, accuracy = 1)) %>% 
         distinct(UseOS, per, n, N)               %>% 
         ggplot(aes(x = reorder(UseOS, -n), y = n)) + 
         geom_col(fill = "light blue", colour = "black") + 
         theme_bw() +
         geom_text(aes(label = n), vjust = -0.5, size = 3) +
         #geom_text(aes(label = paste0(n," (",per,")")), vjust = -0.5, size = 3) +
...
data %>% select(Q9, num_range("Q19_", 1:22))                             %>%
         ...
         pivot_longer(cols = starts_with("Q19"),
                      ...
                      values_to ="discipline")                           %>%
         filter(discipline != 0)                                         %>%
         #distinct(discipline, Q9) %>% 
         mutate(N = n())                                                 %>% 
         group_by(discipline, Q9)                                        %>%
         mutate(n = n())                                                 %>%
         distinct(Q9, discipline, N, n)                                  %>% 
         group_by(Q9)                                                    %>% 
         summarise(Tot = sum(n))                                         %>% 
         arrange(desc(Tot))
```
