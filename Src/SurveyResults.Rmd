...
uids <- c(uids, "839822-839804-91122774")
...
data$Q17[!is.na(data$Q17) & data$Q17 == "PhD student"] <- "University of Bristol"
data$Q17[!is.na(data$Q17) & data$Q17 == "University"] <-  NA # Can't place
...
`r percent(nrow(b)/nrow(data), accuracy = 0.1)` respectively
...
data %>%  select(num_range("Q16_", 1:13), career = Q20) %>%
  ...
  geom_text(aes(label = percent(per, accuracy = 1)), position = position_dodge2(width = 0.9, preserve = "single"), vjust = -0.5, size = 2) +
  theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0))  +
  scale_fill_brewer(palette = "Set1", name = "Career stage")
