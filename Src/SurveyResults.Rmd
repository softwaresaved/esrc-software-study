...
data %>% select(num_range("ImportantDS_", 1:7))                %>% 
         ...
         filter(n > 2)                                         %>% 
         ggplot(aes(x = reorder(importantDS, -n), y = n)) +
...
data %>%  select(num_range("Q19_", 1:22))               %>%
          ...
          geom_text(aes(label = paste0(n, " (",per,")")), stat = "count", vjust = -0.5, size = 2) +
          theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0))
```
