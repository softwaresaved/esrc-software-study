...
data %>% select(id = URN, num_range("Q2_",1:2))                       %>%
         ...
         tally()                                                       %>% 
         arrange(desc(n))                                              %>% 
         kbl(format="pipe", align = c("l","r"), col.names = c("Combined choice", "Number"))
```
...
data %>% select(num_range("Q3_", 1:7), career = "Q20")                     %>%
        ...
        scale_fill_brewer(palette = "Set1", name = "Career stage")
```
### Data sharing segmented by percentage responses at each career stage
