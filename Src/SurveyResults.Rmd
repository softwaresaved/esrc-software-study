...
data %>% select(num_range("Q2_b_",1:6), gender = Q21)      %>%
         ...
         theme_bw() + ylab("Percent") +
         scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
         geom_text(aes(label = scales::percent(per, accuracy = 1)), position = position_dodge2(width = 0.9, preserve = "single"), vjust = -0.35, size = 2)  +
         xlab("Data sources") + 
         theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0)) +
         scale_fill_brewer(palette = "Set2", name = "Gender")
```
...
data %>% select(num_range("Q2_b_",1:6), institutions = CleanLocs)     %>%
         ...
         geom_text(aes(label = ..count..), position = position_stack(),  stat = "count", vjust = -0.35, size = 2)
...
#### Number of NCRM courses by individual
