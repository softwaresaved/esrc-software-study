...
data %>% select(num_range("Q4_",1:26))                              %>%
         ...
         theme(axis.text.x = element_text(angle = 45, size = 10, hjust = 1.0), legend.position = "none")
```
...
using from the survey, `r nrow(a)` individuals have taken NCRM courses or, if we take the `Other` content category
this rises to `r nrow(b)` individuals that is `r percent(nrow(a)/nrow(data), accuracy = 0.1)` or
`r percent(nrow(b)/nrow(data), accuracy = 0.1)` respectively have taken NCRM courses.
...
We can use the above information to normalise contributions so that if a research
discipline is mentioned N times it will only contribute 1/N times.
