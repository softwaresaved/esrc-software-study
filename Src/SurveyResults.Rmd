...
Percentages are given as the fraction from the total for that particular
career stage - note that these may not add to 100% as those that do not
create or reuse data are not included in the graph.
...
data %>% select(num_range("Q2_", 1:2), gender = Q21)                          %>%
         ...
         theme_bw() + scale_y_continuous(labels = scales::percent_format(accuarcy = 1)) +
         xlab("Use of data") + ylab("Percentage") +
         geom_text(aes(x = datause, label = scales::percent(per, accuracy = 1 )), vjust = -0.5,
...
Can produce a plot of the above table.
...
data %>% select(id = URN, num_range("Q2_",1:2))                       %>%
         ...
         tally()                                                       %>% 
         arrange(desc(n))                                              %>% 
         ggplot(aes(x = choices, y = n)) + 
         geom_col(fill = "light blue", colour = "black") + theme_bw() +
...
data %>% select(id = URN, num_range("Q2_",1:2), career = Q20)          %>%
         ...
         xlab("Data use") + ylab("Numbers") +
         geom_text(aes(label = n), position = position_stack(0.5), size = 2) +
         scale_x_discrete(labels = c("Create", "Create and Reuse", "Reuse")) +
         scale_fill_brewer(palette = "Set1", name = "Career stage")
```
...
data %>% select(id = URN, num_range("Q2_",1:2), career = Q20)          %>%
         ...
         group_by(choices, career)                                     %>%
         mutate(per = n()/N)                                           %>% 
         distinct(choices, career, per)                                %>% 
         ggplot(aes(x = choices, y = per, fill = career)) + 
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"), colour = "black") + 
         theme_bw() +
...
data %>% select(num_range("Q2_b_",1:6), career = Q20)      %>%
         ...
         geom_bar(colour = "black") +
         theme_bw() + ylab("Numbers") +
         geom_text(aes(label = ..count..), position = position_stack(0.5), stat = "count", size = 2)  +
         xlab("Data sources") + 
         theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0)) +
         scale_fill_brewer(palette = "Set1", name = "Career stage")
```
...
Percentages do not add up to 100% as null values for the population not shown on
the graph.
...
data %>% select(num_range("Q2_b_",1:6), institutions = CleanLocs)     %>%
         ...
         ylab("Numbers") + xlab("Institutions") + labs(fill = "Data sources") +
         theme(axis.text.x = element_text(size = 8, angle = -90, vjust = 0.15, hjust=0),
               legend.text=element_text(size = 6)) +
         geom_text(aes(label = ..count..), 
                   position = position_stack(),  stat = "count", vjust = -0.35, size = 2)
...
Percentages at a career stage will not add to 100% as the null values are not 
shown.
...
data %>% select(num_range("Q3_", 1:7), career = "Q20")                     %>%
        ...
        xlab("Data sharing habits") + ylab("Numbers") +
        scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
        geom_text(aes(x = DataSharing, label = scales::percent(per, accuracy = 1)), vjust = -0.5,
                        position = position_dodge2(width = 0.9, preserve = "single"), colour = "black", size = 2) +
        scale_fill_brewer(palette = "Set1", name = "Career stage")
```
...
### Software used by percentage career stage population
...
data %>% select(num_range("Q4_",1:26), num_range("Q19_", 1:22))              %>%
         ...
         geom_tile(colour = "black") + scale_fill_continuous(high = "#132B43", low = "#56B1F7") +
         theme_minimal() +
         theme(axis.text.x = element_text(angle = -90, vjust = 0.15, hjust=0)) +
...
each career stage. Only software that has been nominated more than twice
are shown. Note that an individual may have suggested more than one
piece of software - this has not been normalised.
...
data %>% select(num_range("Q7_", 1:7))              %>%
         ...
         theme_bw() +
         theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0)) +
         xlab("How Software is being maintained") + ylab("Numbers") +
         geom_text(aes(label = ..count..), stat = "count", vjust = -0.5, size = 2)
```
...
data %>% select(num_range("Q7_", 1:7), career = Q20)  %>%
         ...
         geom_text(aes(label = ..count..), 
                   position = position_stack(0.5), stat = "count", size = 2) +
         scale_fill_brewer(palette = "Set1", name = "Career stage")
...
# This takes into account the "Other" category
...
Out of the `r nrow(data)` individuals that took the survey, `r nrow(a)`
individuals have taken NCRM courses or, if we take the `Other` content
this rises to `r nrow(b)` individuals that is
`r scales::percent(nrow(a)/nrow(data), accuracy = 0.1)` or
`r scales::percent(nrow(b)/nrow(data), accuracy = 0.1)` respectively
have taken NCRM courses.
...
data %>% select(starts_with("Q11_b_"), career = Q20) %>%
         pivot_longer(cols = starts_with("Q11_b_"),
                      names_to = NULL,
                      values_to = "ncrm")              %>%
         filter(ncrm != 0)                             %>%
...
data %>% select(starts_with("Q11_b_"), gender = Q21) %>%
         pivot_longer(cols = starts_with("Q11_b_"),
                      names_to = NULL,
                      values_to = "ncrm")              %>%
         filter(ncrm != 0)                             %>%
...
data %>% select(id = URN, starts_with("Q11_b_"), career = Q20) %>%
         ...
         filter(ncrm != 0)                             %>%
         group_by(id)                                  %>% 
         mutate(N = n())                               %>% # Number of courses taken by individual
         group_by(career, N)                           %>% 
         mutate(n = n())                               %>% # Number of individuals that take N courses
         ungroup()                                     %>% 
         distinct(career, n, N)                        %>% 
         ggplot(aes(x = N, y = n, fill = career)) +
         geom_col(colour = "black") + theme_bw() +
         scale_fill_brewer(palette = "Set1", name = "Career stage") +
         theme(legend.position = "none")  + ylab("Number of people") +
         #scale_x_discrete() +
...
### Recognition segmented by career stage percentage of population
...
data %>%  select(num_range("Q16_", 1:13), career = Q20) %>%
  ...
  geom_text(aes(label = scales::percent(per, accuracy = 1)), position = position_dodge2(width = 0.9, preserve = "single"), vjust = -0.5, size = 2) +
  theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0))  +
  scale_fill_brewer(palette = "Set1", name = "Career stage")
...
data %>% select(Institution = CleanLocs) %>%
        ...
        head(n = 12)                     %>% 
        kbl(format="pipe", align = c("l","r"),
                col.names = c("Institution", "Numbers"))
```
...
funding body - each of these is given a count of 1.
...
data %>%  select(num_range("Q19_", 1:22), gender = "Q20") %>%
          pivot_longer(cols = num_range("Q19_", 1:22),
                       names_to = "questions",
                       values_to = "disciplines")                 %>%
          filter(disciplines != 0)                                %>%
...
data %>%  select(career = Q20)     %>%
          ...
          theme_void() +
          geom_text(aes(y = per, label = scales::percent(per, accuracy = 1)), colour = "black", position = position_stack(0.5), size = 2.5) +
          scale_fill_brewer(palette = "Set1", name = "Career stage")
```
...
data %>% select(ethnicity = "Q22")    %>%
         ...
         geom_text(aes(y = ethnicity, label = n), hjust = -0.75, size = 2)
...
data %>% select(ethnicity = "Q22", career = Q20)    %>%
         ...
         scale_y_discrete(expand = expansion(add = 0.75), 
                          labels = function(x) stringr::str_wrap(x, width = 35)) + 
         theme(axis.text.y = element_text(size = 8)) +
         geom_text(aes(label = scales::percent(per, accuracy = 1)), hjust = -0.5,
...
data %>% select(dis = Q23, career = Q20)    %>% 
         ...
         geom_text(aes(x = dis, label = scales::percent(per, accuracy = 1)),
