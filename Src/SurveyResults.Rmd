...
data %>% select(num_range("Q2_b_",1:6))      %>%
         ...
         group_by(ds)                        %>% 
         mutate(n = n())                     %>% 
         ggplot(aes(x = reorder(ds, -n), fill = ds)) + geom_bar(colour = "black") +
         theme_bw() + ylab("Number") +
         geom_text(aes(label = ..count..), stat = "count", vjust = 2.0, size = 3, colour ="white") +
         theme(axis.text.x = element_text(angle = -60, vjust = 0.5, hjust=0)) +
         xlab("Data sources") + scale_fill_manual(values = brewer.pal(7, "Blues")) +
