...
data %>% select(num_range("Q4_",1:26), career = "Q20")                       %>%
         ...
         geom_bar(colour = "black") +
         theme_bw() +
         theme(axis.text.x = element_text(angle = -90, vjust = 0.15, hjust=0, size = 8), legend.text=element_text(size = 8)) +
         ylab("Numbers") + xlab("Software used") +
...
data %>% select(num_range("Q4_",1:26), num_range("Q19_", 1:22))              %>%
         ...
         theme_bw() +
         theme(axis.text.x = element_text(angle = -90, vjust = 0.15, hjust=0), legend.text=element_text(size = 8)) +
...
### Frequency of the most important software
...
bind_cols(data["Q20"], important_software)                        %>% 
                       ...
                       ylab("Numbers") + xlab("Most important software") +
                       scale_fill_brewer(palette = "Set1", name = "Career stage") +
                       theme(axis.text.x = element_text(size = 8), legend.text=element_text(size = 8))  

```

### Software used by % career stage

Only show software that has more than 2 votes.
...
data %>% select(starts_with("Q6_b"), career = Q20)  %>%
         ...
         theme_bw() +
         scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
         geom_text(aes(label = scales::percent(per, accuracy = 1)), vjust = -0.5,
...
### How skills are acquired by career stage
...
data %>% select(num_range("Q11_", 1:12), career = Q20)   %>%
         ...
         mutate(per = n()/N)                             %>% 
         distinct(career, skills, per)                   %>% 
         ggplot(aes(x = skills, y = per, fill = career)) + 
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"),
                  colour = "black") +
         theme_bw() +
         scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
...
### How skills are acquired by gender
...
### How skills are acquired by % gender pop
...
data %>% select(num_range("Q11_", 1:12), gender = Q21)   %>%
         ...
         mutate(per = n()/N)                             %>% 
         distinct(gender, skills, per)                   %>% 
         ggplot(aes(x = skills, y = per, fill = gender)) + 
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"),
                  colour = "black") +
         theme_bw() +
         scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
...
data %>% select(num_range("Q13_4_",1:5))            %>%
         ...
         geom_text(aes(label = ..count..), stat = "count", vjust = -0.5, size = 3) +
         theme_bw() + ylab("Numbers") +
         xlab("Insufficient incentive learn how s/w is funded, managed and licensed")
```
