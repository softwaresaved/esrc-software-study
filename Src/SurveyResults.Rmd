...
# Function to list other variations
listOther <- function(Quest){
  data %>% select(other = all_of(Quest))          %>%
           filter(!is.na(other))                  %>% 
...
Each segment represents a percentage of the total from a particular career stage.
...
data %>% select(num_range("Q2_", 1:2), career = Q20)                          %>%
         ...
         geom_text(aes(x = datause, label = scales::percent(per, accuracy = 0.1)), 
                   position = position_dodge(width = 0.9), vjust = -0.235, size = 2) +
         scale_fill_brewer(palette = "Set1", name = "Career stage")
```
...
Commented out the map as it does not generate output for docx format.
...
if(interactive()){
 ...
 cat("Cannot generate an html widget for word.")
}
