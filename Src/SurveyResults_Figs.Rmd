...
This code analyses the results of the data from "[A survey of digital methods in the economics and social sciences research](https://github.com/softwaresaved/esrc-software-study/blob/main/Docs/esrc-survey.pdf)" and produces a superset of the figures included in a report for the
ESRC. Analysis and interpretation is carried out in the report and not here.

## Setup

Read in the packages that are required to read and process the data.
...
Read in the raw data and ancillary files.
...
data0 <- read_csv("../Data/survey-results.csv", col_types = ColTypes)
...
# Make a figures directory if it does not exist
if(!dir.exists("Figures")){
  print("Creating a Figures subdirectory.")
  dir.create("Figures")
...
Process the data to be ready for the output.
...
# 26	Other
software <-  c("AnimationStoryborarding", "Audiotools", "AuthoringPublishing",
...
# Add a flag to indicate whether funding has come from the ESRC
if(!("esrcfunding" %in% colnames(data))){ # Check the column does not already exist

  funding %>% rowwise()                                                        %>%
              filter(any(!is.na(c_across(Q18_1:Q18_7))))                       %>% 
              mutate(esrcfunding = any(grepl("ESRC", c_across(Q18_1:Q18_7))))  %>% 
              select(URN, esrcfunding)                                         %>% 
              right_join(data, by = "URN") -> data

}

# Count instances that only state having ESRC funding and no other
# funding %>% pivot_longer(cols = starts_with("Q18"),
#                          names_to = NULL,
#                          values_to = "fund")      %>% 
#             filter(!is.na(fund))                  %>% 
#             group_by(URN)                         %>% 
...
if(!("datause" %in% colnames(data))){
    ...
    data %>% select(URN, num_range("Q2_",1:2))                             %>%
             ...
             mutate(datause = case_when(datause == "create_reuse" ~ "Create-Reuse", 
                                        ...
                                        datause == "0_0" ~ "None"))       %>% 
             select(-ds)                                                  %>% 
             distinct(URN, datause) %>%  ungroup() -> dd
  
    # Add datause to the original data set if not already there
    data <- data %>% left_join(dd, by = "URN")

}

# Write information about sharing software back to the data tibble
# Take the option that has the the widest scope (in this instance 
# the rightmost value in the list) - this is picked up by the 
# slice(n()) line.
if(!("sharesw" %in% colnames(data))){

    data %>% select(URN, num_range("Q10_", 1:5))         %>% 
             pivot_longer(cols = num_range("Q10_", 1:5),
                          names_to = NULL,
                          values_to = "sharesw")         %>% 
             filter(sharesw != 0)                        %>% 
             group_by(URN)                               %>% 
...
Question: **Do you create or re-use data to undertake your research (Please check
all that apply)**
...
Displaying the responses as a percentage of each career stage.
...
### Data use by gender and career stage
...
This graph isolates the categories into those that specified create-only (Create), reuse-only (Reuse) and both creating and reusing data (Create-Reuse).
...
data %>% select(id = URN, num_range("Q2_",1:2))                        %>%
         ...
         scale_x_discrete(labels = c("Create", "Create and Reuse", "Reuse")) -> Fig13

# Print the result
print(Fig13)

# Save the plot to file
suppressMessages(ggsave("Figures/Figure13.png", dpi = 360))

```


### Fig 15: Combined plot by % career stage
...
data %>% select(URN, num_range("Q2_",1:2), career = Q20)               %>%
         ...
         scale_fill_brewer(palette = "Set1", name = "Career stage") -> Fig15

# Print the plot
print(Fig15)

# Save the plot to file
suppressMessages(ggsave("Figures/Figure15.png", dpi = 360))
...
data %>% select(URN, num_range("ImportantDS_", 1:7))          %>% 
         ...
         theme(axis.text.x = element_text(angle = -60, vjust = 0.5, hjust=0, size = 10), legend.position = "None") +
         xlab(my$lab) + ylab("Numbers") -> Fig10

# Print out the results
print(Fig10)

# Save the results to file
suppressMessages(ggsave(filename="Figures/Figure10.png", dpi = 360))

```

### Fig 14: important data sources by data use
...
data %>% select(URN, num_range("ImportantDS_", 1:7), datause)  %>% 
         ...
         xlab(my$lab) + ylab("Numbers") + labs(fill = "Data use") -> Fig14

# Print the plot
print(Fig14)

# Save the plot
suppressMessages(ggsave("Figures/Figure14.png", dpi = 360))

```

### Fig 11: Important data sources by % career
...
data %>% select(num_range("ImportantDS_", 1:7), career = Q20)  %>% 
         ...
         scale_fill_brewer(palette = "Set1", name = "Career stage") -> Fig11

# Print the plot
print(Fig11)

# Save the file
suppressMessages(ggsave("Figures/Figure11.png", dpi = 360))

```

### Fig 12: Important data sources by % gender
...
data %>% select(num_range("ImportantDS_", 1:7), gender = Q21)  %>% 
         ...
         theme(axis.text.x = element_text(angle = -45, hjust=0, size = 8)) +
         xlab("Data sources") + ylab("Percent") +
         scale_fill_brewer(palette = "Set2", name = "Gender") -> Fig12

# Print the results to screen
print(Fig12)

# Save the figure to file
suppressMessages(ggsave("Figures/Figure12.png", dpi = 360))
...
### Fig16: Reusing existing data responses
...
data %>% select(URN, num_range("Q2_b_",1:6))            %>%
         ...
         xlab(my$lab) + ylim(0, 80) +
         theme(axis.text.x = element_text(angle = -60, vjust = 0.5, hjust=0, size = 8)) +
...
# Save the plot
suppressMessages(ggsave("Figures/Figure16.png", dpi = 360))
...
### Fig17: Reusing existing data by % career stage
...
data %>% select(num_range("Q2_b_",1:6), career = Q20)      %>%
         ...
         scale_fill_brewer(palette = "Set1", name = "Career stage") -> Fig17

# Print the plot to screen
print(Fig17)

# Save the plot to file
suppressMessages(ggsave("Figures/Figure17.png", dpi = 360))
...
data %>% select(num_range("Q2_b_",1:6), gender = Q21)      %>%
         ...
         geom_text(aes(label = after_stat(count)), position = position_stack(0.5), stat = "count", size = 3)  +
         xlab("Data sources") + 
...
### Fig 18: reusing data by research discipline
...
data %>% select(URN, num_range("Q2_b_",1:6), num_range("Q19_", 1:22))        %>%
         ...
         theme(axis.text.y = element_text(size = 8)) + 
         geom_text(aes(label = percent(n/NN, accuracy = 0.1)), size = 3, colour = "black")  +
         scale_fill_distiller(palette = "Blues", direction = 1) +
         theme(axis.text.x = element_text(angle = -70, vjust = 0.5, hjust=0, size = 8),
               legend.position = "None") -> Figure18

# Print the plot
print(Figure18)

# Save the plot to file
suppressMessages(ggsave("Figures/Figure18.png", dpi = 360))
...
data %>% select(URN, num_range("Q3_", 1:7))                           %>%
         ...
         theme(legend.position = "none") -> Figure19

# Print the plot
print (Figure19)

# Save the plot to file
suppressMessages(ggsave("Figures/Figure19.png", dpi = 360))

```

### Fig 23: response distribution and ESRC funding
...
data %>% select(URN, num_range("Q3_", 1:7), esrcfunding)                          %>%
         mutate(esrcfunding = as.character(esrcfunding)) %>% 
         pivot_longer(cols = num_range("Q3_", 1:7),
                      names_to = NULL,
                      values_to = "DataSharing")                                  %>%
         filter(DataSharing != 0)                                                 %>%
         mutate(N = n_distinct(URN))                                              %>% 
         group_by(DataSharing, esrcfunding)                                       %>% 
         mutate(n = n())                                                          %>% 
         mutate(per = percent(n/N, accuracy = 0.1))                               %>% 
         mutate(esrcfunding = ifelse(is.na(esrcfunding), "Unknown", esrcfunding)) %>% 
         distinct(n, per, DataSharing, esrcfunding)                               %>% 
         ggplot(aes(x = reorder(DataSharing, -n), y = n, fill = esrcfunding)) +
         geom_col(colour = "black", position = position_dodge2(width = 0.9, preserve = "single")) + 
         theme_bw() + 
         theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0, size = 12)) +
         xlab(my$lab) + ylab("Numbers") + ylim(0, 60) +
         geom_text(aes(label = paste0(n, "\n(", per,")")),position = position_dodge2(width = 0.9, preserve = "single"), 
                   vjust = -0.25, size = 2) +
         scale_fill_manual(name = "ESRC funding", labels = c("no-ESRC funding","ESRC funding", "Unknown"), values = c("red", "green","blue")) -> Figure23

# Print the plot
print(Figure23)

# Save the plot to file
suppressMessages(ggsave("Figures/Figure23.png", dpi = 360))
...
### Fig 20: data sharing by % career stage
...
data %>% select(num_range("Q3_", 1:7), career = "Q20")                     %>%
        ...
        geom_text(aes(x = DataSharing, label = percent(per, accuracy = 1)), vjust = -0.5,
                      position = position_dodge2(width = 0.9, preserve = "single"), 
                      colour = "black", size = 3) + 
        scale_fill_brewer(palette = "Set1", name = "Career stage") -> Fig20

# Print the plot
print(Fig20)

# Save the plot to file
suppressMessages(ggsave("Figures/Figure20.png", dpi = 360))

```

### Fig 22: data sharing by % data use
...
data %>% select(num_range("Q3_", 1:7), datause)                            %>%
        ...
        geom_text(aes(x = DataSharing, label = percent(per, accuracy = 1)), vjust = -0.5,
                      position = position_dodge2(width = 0.9, preserve = "single"), 
                      colour = "black", size = 3) + 
        scale_fill_brewer(palette = "PuRd", name = "Data use") -> Figure22

# Plot the diagram
print(Figure22)

# Save the plot to file
suppressMessages(ggsave("Figures/Figure22.png", dpi = 360))

```

### Data sharing by % data use and career

```{r sharing_data_datause_career_per}

data %>% select(num_range("Q3_", 1:7), datause, career = Q20)               %>%
         filter(!(career %in% c("-", "Other")))                             %>% 
         group_by(datause)                                                  %>% 
         mutate(N = n())                                                    %>%
         pivot_longer(cols = starts_with("Q3_"), 
         ...
         group_by(DataSharing)                                              %>%
         mutate(M = n())                                                    %>% 
         group_by(DataSharing, datause, career)                            %>% 
         mutate(per = n()/N)                                                %>% 
         distinct(datause, DataSharing, per, M, career)                     %>% 
         ggplot(aes(x = reorder(DataSharing, -M), y = per,fill = datause)) +
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"), 
                 colour = "black", na.rm = TRUE) + 
        theme_bw() + 
...
### Data sharing by % data use and gender

```{r sharing_data_datause_career_per2}

data %>% select(num_range("Q3_", 1:7), datause, gender = Q21)               %>%
         filter(!(gender %in% lowstatsgender))                              %>% 
         group_by(datause)                                                  %>% 
         mutate(N = n())                                                    %>%
         pivot_longer(cols = starts_with("Q3_"), 
         ...
         group_by(DataSharing)                                              %>%
         mutate(M = n())                                                    %>% 
         group_by(DataSharing, datause, gender)                             %>% 
         mutate(per = n()/N)                                                %>% 
         distinct(datause, DataSharing, per, M, gender)                     %>% 
         ggplot(aes(x = reorder(DataSharing, -M), y = per,fill = datause)) +
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"), 
                 colour = "black", na.rm = TRUE) + 
        theme_bw() + 
...
### Fig 21: sharing by gender and career
...
data %>% select(num_range("Q3_", 1:7), gender = Q21, career = Q20)         %>%
        ...
        geom_text(aes(x = DataSharing, label = percent(per, accuracy = 1)), vjust = -0.5,
                      ...
                      colour = "black", size = 3) + 
        scale_fill_brewer(palette = "Set2", name = "Gender") + 
        facet_wrap( ~ career) -> Figure21

# Print the plot
print(Figure21)

# Save the plot
suppressMessages(ggsave("Figures/Figure21.png", dpi = 360))

```


### Sharing data and sharing s/w

```{r sharing_swVsdata}

data %>% select(URN, sharesw, num_range("Q3_", 1:7))        %>%
         pivot_longer(cols = num_range("Q3_", 1:7),
                      names_to = NULL,
                      values_to = "DataSharing")            %>%
         filter(DataSharing != 0)                           %>% 
         distinct(URN, DataSharing, sharesw)                %>% 
         group_by(DataSharing, sharesw)                    %>% 
         tally()                                            %>%
         ggplot(aes(x = sharesw, y = DataSharing, fill = n)) +
         theme_bw() + 
         geom_tile() +
         theme(legend.position = "None", axis.text.x = element_text(angle = -90, hjust = 0)) +
         ylab("Data sharing") + xlab("Software sharing") + 
         scale_fill_distiller(palette = "Blues", direction = 1) +
...
### Fig 24: software used in research chart
...
data %>% select(URN, num_range("Q4_",1:26))                         %>%
         ...
         geom_col(colour = "black") +
         theme_bw() + ylim(0, 140) + scale_fill_distiller(palette = "Blues", direction = 1) +
         ylab("Numbers") + xlab(my$lab) +
         geom_text(aes(label = paste0(n,"\n(", percent(n/N, accuracy = 0.1),")")), vjust = -0.5, size = 2) +
         theme(axis.text.x = element_text(angle = 60, size = 10, hjust = 1.0), legend.position = "none") -> Figure24

# Print the plot
print(Figure24)

# Save the plot to a file
suppressMessages(ggsave("Figures/Figure24.png", dpi = 360))

```

### Fig 27: choice correlations
...
# Had to save this manually by pasting in the console and using the RStudio saving mechanism.
corrplot(cor(q), method="circle", type= "lower", diag = FALSE, number.cex = 5)
...
### Fig 25: software used by career percent of software use
...
data %>% select(num_range("Q4_",1:26), career = Q20)                         %>%
         ...
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"), colour = "black") +
         theme_bw() +
         theme(axis.text.x = element_text(angle = -45, vjust = 0.15, hjust=0, size = 8),
               legend.text=element_text(size = 8)) +
         ...
         geom_text(aes(x = software, y = per, label = percent(per, accuracy = 1)), 
                 ...
                 size = 2) +
         scale_fill_brewer(palette = "Set1", name = "Career stage") -> Figure25

# Print the plot
print(Figure25)

# Save the plot to file
suppressMessages(ggsave("Figures/Figure25.png", dpi = 360))
...
data %>% select(num_range("Q4_",1:26), career = Q20, gender = Q21)                         %>%
         ...
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"), 
                  colour = "black") +
         theme_bw() +
         scale_y_continuous(labels = percent_format(accuracy = 1), limits = c(0, 0.36)) +
         ...
         geom_text(aes(label = percent(per, accuracy = 1)), position = position_dodge2(width = 0.9, preserve = "single"), 
                   size = 2, vjust = -0.5) +
         ylab("Percent") + xlab("Software used") + 
         scale_fill_brewer(palette = "Set2", name = "Gender") + facet_wrap(~career) -> Fig26

# Print file
print(Fig26)

# Save the plot to file
suppressMessages(ggsave("Figures/Figure26.png", dpi = 360))

```

### Fig 28: software used by discipline
...
data %>% select(URN, num_range("Q4_",1:26), num_range("Q19_", 1:22))         %>%
         ...
         geom_text(aes(label = percent(n, accuracy = 0.1)), colour = "black", size = 2) +
         theme(axis.text.x = element_text(angle = -90, vjust = 0.15, hjust=0),
               legend.position = "None") +
         ylab("Research discipline") + xlab("Software used") -> Fig28

# Print the plot
print(Fig28)

# Save the plot to file
suppressMessages(ggsave("Figures/Figure28.png", dpi = 360))
...
### Fig 29: most important software
...
important_software %>% select(URN, starts_with("Q5"))        %>%
                       ...
                       theme_bw() +
                       theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0)) +
                       geom_text(aes(x = software, label = paste0(n, "\n(", percent(n/N, accuracy = 1),")")), 
                                 vjust = -0.5, size = 3) +
                       ylab("Numbers") + xlab(my$lab) + ylim(0, 63) +
                       theme(axis.text.x = element_text(size = 8), legend.position = "None")  -> Fig29

# Print the plot to screen
print(Fig29)

# Save the plot to file
suppressMessages(ggsave("Figures/Figure29.png", dpi = 360))
...
data %>% select(os = Q6, career = Q20, gender = Q21)    %>%
         ...
         theme_bw() +
         xlab("Use of Open Source") + 
         scale_y_continuous(labels = percent_format(accuracy = 1), limits = c(0, 0.28)) +
         ylab("Percent") +
         geom_text(aes(label = paste0(n," (", per,")")),
                   position = position_dodge2(width = 0.9, preserve = "single"),
                   vjust = -0.5, size = 4) +
...
data %>% select(URN, num_range("Q6_a_", 1:8))                   %>%
         pivot_longer(cols = num_range("Q6_a_", 1:8),
                      names_to = NULL,
                      values_to = "ds")                        %>%
         filter(ds != 0)                                       %>%  
         group_by(URN, ds)                                     %>%
...
# ConcernAboutValue is a dummy column to get the right number of rows.
...
# Remove the constructor column
vc$ConcernAboutValue <-  NULL

# Produce a contingency table
tt <- names(vc)                                          # Names to be used for the pivot wider
m <-  names(sort(diag(as.matrix(vc)), decreasing = TRUE)) # Order to be used for the axes
vc %>% mutate(xx = tt)                  %>% 
       pivot_longer(cols = all_of(tt),
                    names_to = "yy",
                    values_to = "n")    %>% 
       ggplot(aes(x = factor(xx, levels=m), y = factor(yy, levels = rev(m)), fill = n)) +
       theme_bw() + geom_tile() +
       scale_fill_distiller(palette = "Blues", direction = 1) +
       theme(axis.text.x = element_text(angle = -60, hjust = 0),
            legend.position = "None") +
       geom_text(aes(label = n), size = 4) +
       xlab("Use of OS") + ylab("Use of OS")
...
data %>% select(starts_with("Q6_a"), career = Q20)   %>%
         ...
         geom_bar(colour = "black") + 
         theme_bw() +
         geom_text(aes(label = after_stat(count)), stat = "count", position = position_stack(0.5), size = 3) +
         theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0)) +
...
### Using OS by % gender and career

```{r reason4os_gender_car_per}
...
Showing numbers instead of percentages.

```{r reason4os_gender_car_num}
...
data %>% select(starts_with("Q6_a"), career = Q20, gender = Q21)   %>%
         ...
         group_by(UseOS, gender, career)                           %>% 
         mutate(n = n())                                           %>%
         distinct(UseOS, gender, n, M, career)                     %>% 
         ggplot(aes(x = reorder(UseOS, -M), y = n, fill = gender)) + 
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"), colour = "black") + 
         theme_bw() + ylim(0, 18) +
         #scale_y_continuous(labels = percent_format(accuracy = 1)) +
         geom_text(aes(label = n), 
...
data %>% select(URN, starts_with("Q6_a"), career = Q20, gender = Q21)      %>%
         filter(!(gender %in% lowstatsgender))                             %>% 
         filter(!(career %in% c("-","Other")))                             %>% 
         group_by(gender)                                                  %>% 
         mutate(N = n())                                                   %>% 
         ...
         group_by(gender, career)                                          %>% 
         mutate(n = n_distinct(URN))                                       %>% 
         mutate(career = factor(career, ordered = TRUE, levels = careero)) %>% 
         mutate(p = percent(n/nn, accuracy = 0.1))                         %>% 
         distinct(career, gender, n, N, nn, p)                             %>% 
         arrange(gender, career)                                           %>% 
         kbl(format = "pipe", 
             col.names = c("Career stage", "Gender", "Gender Tot","Gender in career stage", "Answered", "Percent"))
```
...
data %>% select(URN, starts_with("Q6_b"))         %>%
         ...
         geom_col(colour = "black") + 
         theme_bw() + scale_fill_distiller(palette = "Blues", direction = 1) +
         geom_text(aes(label = paste0(n, " (",percent(n/N, accuracy = 0.1),")")), vjust = -0.5, size = 4) +
         theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0, size = 12), legend.position = "None") +
         xlab(my$lab) + ylab("Numbers")
```
...
data %>% select(starts_with("Q6_b"), career = Q20)        %>%
         ...
         group_by(UseOS, career)                          %>% 
         mutate(n = n())                                  %>% 
         ggplot(aes(x = reorder(UseOS, -m), fill = career)) + 
         geom_bar(colour = "black") + 
         theme_bw() +
...
### Responses by % gender and career

```{r reasons4NotOS_gender_career_per}

data %>% select(starts_with("Q6_b"), career = Q20, gender = Q21) %>%
         filter(!(gender %in% lowstatsgender))                   %>% 
         filter(!(career %in% c("-", "Other")))                  %>% 
         group_by(gender)                                        %>% 
         mutate(N = n())                                         %>% 
         ...
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"), colour = "black") + 
         theme_bw() +
         scale_y_continuous(labels = percent_format(accuracy = 1), limits = c(0, 0.09)) +
         geom_text(aes(label = percent(per, accuracy = 0.1)), vjust = -0.5,
                   position = position_dodge2(width = 0.9, preserve = "single"), size = 4) +
         theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0)) +
         xlab("Reasons for not using Open Source") + ylab("Percent") +
         scale_fill_brewer(palette = "Set2", name = "Gender") +
...
# Create a data frame showing the choices per individual
data %>% select(URN, num_range("Q6_b_", 1:8))                   %>%
         pivot_longer(cols = num_range("Q6_b_", 1:8),
                      names_to = NULL,
                      values_to = "ds")                        %>%
         filter(ds != 0)                                       %>%  
         group_by(URN, ds)                                     %>%
...
data %>% select(URN, num_range("Q7_", 1:7))             %>%
         ...
         group_by(swmaint)                              %>% 
         mutate(n = n())                                %>% 
         distinct(n, N, swmaint)                        %>% 
         ggplot(aes(x = reorder(swmaint,-n), y = n, fill = n)) + 
         geom_col(colour = "black") +
         theme_bw() + scale_fill_distiller(palette = "Blues", direction = 1) +
         theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0, size = 12), legend.position = "None") +
         xlab(my$lab) + ylab("Numbers") +
...
### Software maintainance by % gender and career
...
### Developing and most important software

```{r developing_importantsw}
...
important_software  %>% select(URN, starts_with("Q5"))                   %>% 
                       ...
                       group_by(URN)                                     %>% 
                       mutate( contrib = 1/n())                          %>% 
                       left_join(data %>% select(URN, Q9), by = "URN")   %>% 
                       group_by(software, Q9)                            %>% 
                       summarise(N = round(sum(contrib), 1), 
                                 .groups = "drop")                       %>% 
                      filter(N > 3)                                      %>% 
                      ggplot(aes(x = Q9, y = N, fill = software)) +
                      geom_col(colour = "black", position = position_stack()) + 
                      theme_bw() +
                      theme(legend.position = "None") +
                      geom_text(aes(label = paste0(software)), 
                                position = position_stack(vjust = 0.5)) +
                      xlab("Do you develop software?") + ylab("Number")
```
Restrict to programming languages.

```{r developing_importantsw_prog}

# Programming languages
proglan <- c("R", ".NET", "Excel", "VBA","Python", "FORTRAN", "Oxmetrics",
             "NetLogo","SPSS","Knime","Stata", "C++", "Jupyter","MATLAB")
...
important_software  %>% select(URN, starts_with("Q5"))                   %>% 
                      ...
                      geom_text(aes(label = paste0(software)), 
                                position = position_stack(vjust = 0.5)) +
                      xlab("Do you develop software?")
```
### Developing and most important software table

```{r developing_importantsw_table_yes}
...
important_software  %>% select(URN, starts_with("Q5"))                   %>% 
                       ...
                       arrange(desc(per))                                %>% 
                       mutate(per = percent(per, accuracy = 0.1))        %>% 
                       kbl(format="pipe", align = c("l","r"), col.names = c("Software", "Percent"))
```

```{r developing_importantsw_table_no}
...
# Calculate an x label with the folks that filled in this question.
data %>% select(URN, num_range("Q10_", 1:6))  %>% 
         pivot_longer(cols = num_range("Q10_", 1:6),
                      names_to = NULL,
                      values_to = "swuse")              %>%
         filter(swuse != 0)                             %>%
...
data %>% select(URN, num_range("Q10_", 1:6))            %>%
         ...
         filter(swuse != 0)                             %>%
          ...
          mutate(n = n())                               %>%
          distinct(n, N, swuse)                         %>%
         ggplot(aes(x = reorder(swuse, -n), y = n, fill = n)) +
         geom_col(colour = "black") + scale_fill_distiller(palette = "Blues", direction = 1) +
         theme_bw() +
         xlab(my$lab) + ylab("Numbers") +
         geom_text(aes(label = paste0(n, " (", percent(n/N, accuracy = 1),")")), vjust = -0.5, size = 4) +
...
data %>% select(URN, num_range("Q10_", 1:5))         %>% 
         ...
         group_by(sharesw)                           %>%
         mutate(n = n())                             %>% 
         distinct(n, N, sharesw)                     %>% 
         ggplot(aes(x = reorder(sharesw, -n), y = n, fill = n)) + 
         geom_col(colour = "black") + scale_fill_distiller(palette = "Blues", direction = 1) +
         theme_bw() +
         xlab(my$lab) + ylab("Numbers") +
         geom_text(aes(label = paste0(n, " (", percent(n/N, accuracy = 1),")")), vjust = -0.5, size = 4) +
...
data0 %>% select(num_range("Q10_", 1:6)) -> q  
names(q) <- swuse
...
### Sharing software by career

```{r sharing_career}

data %>% select(num_range("Q10_", 1:6), career = Q20)   %>%
         filter(!(career %in% c("-", "Other")))         %>% 
         pivot_longer(cols = num_range("Q10_", 1:6),
                      names_to = NULL,
                      values_to = "swuse")              %>%
         ...
         mutate(n = n())                                %>% 
         ggplot(aes(x = reorder(swuse, -M), fill = career)) + 
         geom_bar(colour = "black") +
         theme_bw() +
...
### Sharing software by % career stage

```{r sharing_career_per}

data %>% select(URN, num_range("Q10_", 1:6), career = Q20)   %>%
         filter(!(career %in% c("-", "Other")))              %>% 
         group_by(career)                                    %>%
         mutate(N = n())                                     %>%
         pivot_longer(cols = num_range("Q10_", 1:6),
         ...
         group_by(swuse, career)                             %>%
         mutate(per = n()/N)                                 %>% 
         distinct(swuse, career, per, M)                     %>% 
         ggplot(aes(x = reorder(swuse, -M), y = per, fill = career)) + 
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"), colour = "black") +
         theme_bw() +
         scale_y_continuous(labels = percent_format(accuracy = 1)) +
...
Look at the procssed data which only takes the last value (most liberal sharing of software) option.

```{r sharing_career_per1}

data %>% select(sharesw, career = Q20)                       %>%
         filter(!(career %in% c("-", "Other")))              %>% 
         group_by(career)                                    %>%
         mutate(N = n())                                     %>%
         filter(!is.na(sharesw))                             %>%
         group_by(sharesw)                                   %>% 
         mutate(M = n())                                     %>% 
         group_by(sharesw, career)                           %>%
         mutate(n = n())                                     %>% 
         distinct(sharesw, career, n, N, M)                  %>% 
         ggplot(aes(x = reorder(sharesw, -M), y = n/N, fill = career)) + 
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"), colour = "black") +
         theme_bw() +
         scale_y_continuous(labels = percent_format(accuracy = 1)) +
         xlab("Who do you share your software with?") + ylab("Percent") +
         geom_text(aes(label = percent(n/N, accuracy=1)), vjust = -0.5,
                   position = position_dodge2(width = 0.9, preserve = "single"),  size = 4) +
         theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0)) +
         scale_fill_brewer(palette = "Set1", name = "Career stage")
```
...
### Sharing software by gender

```{r sharing_gender}

data %>% select(num_range("Q10_", 1:6), gender = Q21)   %>%
         filter(!(gender %in% lowstatsgender))          %>% 
         pivot_longer(cols = num_range("Q10_", 1:6),
                      names_to = NULL,
                      values_to = "swuse")              %>%
...
data %>% select(num_range("Q10_", 1:6), gender = Q21)   %>%
         ...
         group_by(swuse, gender)                        %>%
         mutate(per = n()/N)                            %>% 
         distinct(swuse, gender, per, M)                %>% 
         ggplot(aes(x = reorder(swuse, -M), y = per, fill = gender)) + 
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"), colour = "black") +
         theme_bw() +
         scale_y_continuous(labels = percent_format(accuracy = 1)) +
         xlab("Who do you share your software with?") + ylab("Percent") +
...
Using the modified sharing data (accept the most liberal policy):

```{r sharing_gender_per5}

data %>% select(sharesw, gender = Q21)                  %>%
         filter(!(gender %in% lowstatsgender))          %>%
         group_by(gender)                               %>%
         mutate(N = n())                                %>% 
         filter(!is.na(sharesw))                        %>%
         group_by(sharesw)                              %>% 
         mutate(M = n())                                %>% 
         group_by(sharesw, gender)                      %>%
         mutate(n = n())                                %>% 
         distinct(sharesw, gender, n, M, N)             %>% 
         ggplot(aes(x = reorder(sharesw, -M), y = n/N, fill = gender)) + 
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"), colour = "black") +
         theme_bw() +
         scale_y_continuous(labels = percent_format(accuracy = 1)) +
         xlab("Who do you share your software with?") + ylab("Percent") +
...
### Sharing software by gender and career stage

```{r sharing_gender_per3}

data %>% select(num_range("Q10_", 1:6), career = Q20, gender = Q21)   %>%
         group_by(gender)                                             %>%
         mutate(N = n())                                              %>% 
         pivot_longer(cols = num_range("Q10_", 1:6),
                      names_to = NULL,
         ...
         filter(!(career %in% c("-", "Other")))                       %>% 
         filter(!(gender %in% lowstatsgender))                        %>%
         group_by(swuse, gender, career)                              %>%
         mutate(per = n()/N)                                          %>% 
         distinct(swuse, gender, per, career, M)                      %>% 
         ggplot(aes(x = reorder(swuse, -M), y = per, fill = gender)) + 
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"), colour = "black") +
         theme_bw() +
         scale_y_continuous(labels = percent_format(accuracy = 1)) +
         ...
         facet_wrap(~career)
```
Using the modifed interpritation of the data to take the most liberal data sharing option

```{r sharing_gender_per2}

data %>% select(sharesw, career = Q20, gender = Q21)                  %>%
         group_by(gender)                                             %>%
         mutate(N = n())                                              %>% 
         filter(!is.na(sharesw))                                      %>% 
         group_by(sharesw)                                            %>% 
         ...
         filter(!(career %in% c("-", "Other")))                       %>% 
         filter(!(gender %in% lowstatsgender))                        %>%
         group_by(sharesw, gender, career)                            %>%
         mutate(n = n())                                              %>% 
         distinct(sharesw, gender, n, career, M, N)                   %>% 
         ggplot(aes(x = reorder(sharesw, -M), y = n/N, fill = gender)) + 
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"), colour = "black") +
         theme_bw() +
         scale_y_continuous(labels = percent_format(accuracy = 1), limits = c(0, 0.16)) +
         xlab("Who do you share your software with?") + ylab("Percent") +
         geom_text(aes(label = percent(n/N, accuracy = 0.1)), vjust = -0.5,
                   position = position_dodge2(width = 0.9, preserve = "single"),  size = 4) +
         theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0)) +
         scale_fill_brewer(palette = "Set2", name = "Gender") +
         facet_wrap(~career)
...
```{r sharingSW_choices2}

data %>% select(URN, career = Q20, num_range("Q10_", 1:6))            %>%
         pivot_longer(cols = num_range("Q10_", 1:6),
                      names_to = NULL,
                      values_to = "ds")                                %>%
         filter(ds != 0)                                               %>%  
         ...
         filter(n  > 1)                                                %>% 
         arrange(desc(n))                                              %>% 
         kbl(format="pipe", align = c("l","l", "r"), col.names = c("Career", "Combined choice", "Numbers"))
```

```{r sharingSW_choices3}
...
data %>% select(URN, num_range("Q10_", 1:6))            %>%
         ...
         group_by(URN)                                                 %>%
         mutate(choices = paste0(gsub(" ","-", ds), collapse = "_"))   %>% 
         filter(grepl("OwnUseOnly", choices))                          %>% 
         distinct(URN, choices)                                %>% 
         ungroup()                                                     %>% 
         select(-URN)                                                  %>% 
         group_by(choices)                                     %>% 
         ...
         filter(n  > 1)                                                %>% 
         arrange(desc(n))                                              %>% 
         kbl(format="pipe", align = c("l","l", "r"), col.names = c("Combined choice", "Numbers"))
```


### Sharing software contingency table

```{r sharingsw_contingency}
...
vc %>% mutate(xx = tt)                  %>% 
       ...
       theme(axis.text.x = element_text(angle = -60, hjust = 0),
            legend.position = "None") +
       geom_text(aes(label = n), size = 4) +
       xlab("Software sharing") + ylab("Software sharing")

```

```{r sharingsw_contingency_career_gender}

# Create a data frame showing the choices per individual
data %>% select(URN, num_range("Q10_", 1:6), career = Q20, gender = Q21)  %>%
         filter(!(career %in% c("-", "Other")))                           %>%  
         filter(!(gender %in% lowstatsgender))                            %>% 
         #filter(gender == "Man")                                         %>% 
         pivot_longer(cols = num_range("Q10_", 1:6),
...
```{r developing_sharing}

data %>% select(sharesw, Q9)                       %>%
         filter(!is.na(Q9) & !is.na(sharesw))      %>% 
         mutate(N = n())                           %>% 
         group_by(Q9, sharesw)                     %>% 
         mutate(n = n())                           %>% 
         mutate(per = percent(n/N, accuracy = 1))  %>%
         distinct(Q9, N, n, per)                   %>% 
         ggplot(aes(x = reorder(Q9, -n), y = n, fill = sharesw)) + 
         geom_col(colour = "black", position = position_dodge2(width = 0.9, preserve = "single")) +
         theme_bw() + #scale_fill_distiller(palette = "Blues", direction = 1) +
         xlab("Do you develop software?") + ylab("Numbers") + #theme(legend.position = "None") +
         geom_text(aes(label = paste0(n," (", per,")")), 
                   position = position_dodge2(width = 0.9, preserve = "single"), vjust = -0.5, size = 4)
```

### Developing and sharing software by career stage

```{r developing_sharing_career}

data %>% select(sharesw, Q9, career = Q20)         %>%
         group_by(career)                            %>% 
         mutate(N = n())                             %>% 
         filter(!(career %in% c("-", "Other")))      %>% 
         filter(!is.na(Q9) & !is.na(sharesw))        %>% 
         group_by(Q9, sharesw, career)               %>% 
         mutate(n = n())                             %>% 
         mutate(per = percent(n/N, accuracy = 0.1))  %>%
         distinct(Q9, N, n, per, career)             %>% 
         ggplot(aes(x = reorder(Q9, -n), y = n, fill = sharesw)) + 
         geom_col(colour = "black", position = position_dodge2(width = 0.9, preserve = "single")) +
         theme_bw() + 
         scale_y_continuous(breaks= pretty_breaks(), limits = c(0, 13)) +
         xlab("Do you develop software?") + ylab("Numbers") + 
         geom_text(aes(label = paste0(n," (", per,")")), 
                   position = position_dodge2(width = 0.9, preserve = "single"), vjust = -0.5, size = 3) +
        facet_wrap(~career)
...
important_software  %>% select(URN, starts_with("Q5"))                        %>% 
                       ...
                       group_by(URN)                                          %>% 
                       mutate( contrib = 1/n())                               %>% 
                       left_join(data %>% select(URN, sharesw), by = "URN")   %>% 
                       filter(!is.na(sharesw))                                %>% 
                       ungroup()                                              %>% 
                       mutate(N = sum(contrib))                               %>% 
                       group_by(software, sharesw)                            %>% 
                       mutate(n = sum(contrib))                               %>% 
                       filter(n/N > 0.02)                                     %>% 
                       distinct(n, N, sharesw, software)                      %>% 
                       mutate(sharesw = factor(sharesw, ordered = TRUE, 
                                levels = c("ShareWidely", "ShareOnRequest", "CollaboratorsOnly", "RGUseOnly", "OwnUseOnly"))) %>% 
                       ggplot(aes(x = sharesw, y = software, fill = n/N)) +
                       geom_tile(colour = "black", stat = "identity") + 
                       theme_bw() +
                       theme(legend.position = "None") +
                       scale_fill_continuous(type = "viridis") +
                       geom_text(aes(label = paste0(percent(n/N, accuracy = 0.1))), colour = "white") +
                       xlab("Share software") + ylab("Software")
```

### Sharing, developing and most the important software table

```{r q9_developing_importantsw_table_yes}
...
important_software  %>% select(URN, starts_with("Q5"))                   %>% 
                       ...
                       group_by(URN)                                     %>% 
                       mutate(contrib = 1/n())                           %>% 
                       left_join(data %>% select(URN, Q9, sharesw), 
                                 by = "URN")                             %>% 
                       filter(Q9 == "Yes")                               %>%
                       ungroup()                                         %>% 
                       mutate(N = sum(contrib))                          %>% 
                       ...
                       mutate(per = n/N)                                 %>% 
                       filter(per > 0.02)                                %>% 
                       distinct(software, sharesw, per)                  %>% 
                       mutate(per = percent(per, accuracy = 0.1))         %>% 
                       pivot_wider(names_from = sharesw,
                                   values_from = per,
                                   values_fill = "0")                     %>%                    
                       kbl(format="pipe", align = c("l",rep("r", 4)), 
                           col.names = c("Software", "Share Widely", "Share on Request", "Own Use Only", "Collaborators Only"))
...
data %>% select(URN, num_range("Q11_", 1:12))            %>%
         ...
         geom_bar(colour = "black") +
         theme_bw() + scale_fill_distiller(palette = "Blues", direction = 1) +
         xlab(my$lab) + ylab("Numbers") + ylim(0, 129) +
...
data %>% select(num_range("Q11_", 1:12), career = Q20)   %>%
         ...
         geom_text(aes(label = after_stat(count)), position = position_stack(vjust = 0.5), 
                   stat = "count", size = 3) +
         theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0),
               legend.text=element_text(size = 6)) +
         scale_fill_brewer(palette = "Set1", name = "Career stage")
...
### Skills acquisition by % data use

```{r acquiring_skills_datause_per}

data %>% select(num_range("Q11_", 1:12), datause)        %>%
         group_by(datause)                                %>% 
         mutate(N = n())                                 %>% 
         pivot_longer(cols = num_range("Q11_", 1:12),
                      names_to = NULL,
         ...
         group_by(skills)                                %>% 
         mutate(M = n())                                 %>% 
         group_by(datause, skills)                        %>% 
         mutate(per = n()/N)                             %>% 
         distinct(datause, skills, per, M)                %>% 
         ggplot(aes(x = reorder(skills, -M), y = per, fill = datause)) + 
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"),
                  colour = "black") +
         theme_bw() +
         scale_y_continuous(labels = percent_format(accuracy = 1)) +
...
```{r acquiring_skills_gender_per_career}
...
data %>% select(URN, num_range("Q11_b_", 1:22), career = Q20)      %>%
...
data %>% select(URN, starts_with("Q11_b_"), career = Q20)      %>%
...
data %>% select(URN, starts_with("Q11_b_"), career = Q20)      %>%
         pivot_longer(cols = starts_with("Q11_b_"),
                      ...
                      values_to = "ncrm")                      %>%
         filter(ncrm != 0)                                     %>%
         distinct(URN, ncrm, career)                           %>% 
         group_by(career)                                      %>% 
         mutate(n = n(), nid = n_distinct(URN))                %>%
...
data %>% select(URN, starts_with("Q11_b_"), gender = Q21) %>%
         pivot_longer(cols = starts_with("Q11_b_"),
                      names_to = NULL,
                      values_to = "ncrm")                 %>%
         filter(ncrm != 0)                                %>%
...
data %>% select(URN, starts_with("Q11_b_"), career = Q20) %>%
         ...
         group_by(career, N)                              %>% 
         mutate(n = n_distinct(URN))                      %>% # Number of individuals that have taken N courses
         ungroup()                                        %>% 
         distinct(career, n, N)                           %>% 
...
### NCRM and development

Do those that NCRM courses develop software (from the result from Q9)?

```{r ncrm_development}

data %>% select(URN, starts_with("Q11_b_"), dev = Q9)      %>%
         pivot_longer(cols = starts_with("Q11_b_"),
                      names_to = NULL,
                      values_to = "ncrm")                  %>%
         filter(ncrm != 0)                                 %>%
         ...
         mutate(n = n())                                   %>% 
         distinct(dev, n)                                  %>% 
         ggplot(aes(x = dev, y = n, fill = n)) + geom_col(colour = "black") + 
         scale_fill_distiller(palette = "Blues", direction = 1) +
         theme_bw() + xlab("Develop software") + ylab("Number") +
...
data %>% select(URN, starts_with("Q11_b_"), datause)       %>%
         pivot_longer(cols = starts_with("Q11_b_"),
                      names_to = NULL,
                      values_to = "ncrm")                  %>%
         filter(ncrm != 0)                                 %>%
         ...
         mutate(n = n())                                   %>% 
         distinct(datause, n)                              %>% 
         ggplot(aes(x = datause, y = n, fill = n)) + 
         geom_col(colour = "black") + 
         scale_fill_distiller(palette = "Blues", direction = 1) +
         theme_bw() + xlab("datause") + ylab("Number") +
         geom_text(aes(label = n), vjust = -0.5) + 
         theme(legend.position = "None")
  
```

### NCRM and institutions

```{r ncrm_institution}

data %>% select(URN, starts_with("Q11_b_"), institution = Q17)    %>%
         pivot_longer(cols = starts_with("Q11_b_"),
                      names_to = NULL,
                      values_to = "ncrm")                         %>%
         filter(ncrm != 0)                                        %>%
         distinct(URN, institution)                               %>% 
         mutate(institution = ifelse(is.na(institution), "Unknown", institution)) %>% 
         group_by(institution)                                    %>% 
         tally()                                                  %>% 
         arrange(desc(n))                                         %>% 
         kbl(format = "pipe", col.names = c("Institution", "Number"))
...
```{r q12_recognition_careerStage_per}
...
data %>%  select(rew = Q12, gender = Q21)        %>%
          ...
          geom_bar(colour = "black") +
          theme_bw() +
          xlab("Development/Maintenance of research software is rewarded/recognised") +
          ylab("Numbers") +
          geom_text(aes(label = after_stat(count)), position = position_stack(vjust = 0.5), 
                   stat = "count", size = 3) +
          theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0)) +
...
```{r q12_recognition_gender_per_career}
...
```{r q12_recognition_swdev}

data %>%  select(rew = Q12, dev = Q9)  %>%
          filter(rew != "-")           %>%
          filter(!is.na(dev))          %>% 
          mutate(N = n())              %>% 
          group_by(rew, dev)           %>% 
          mutate(n = n())              %>% 
          distinct(n, N, rew, dev)     %>% 
          ggplot(aes(x = rew, y = n, fill = dev)) +
          geom_col(colour = "black")       +
          theme_bw() +
          xlab("Is software development recognised/rewarded?") +
          ylab("Number") + labs(fill="Develop software") +
          geom_text(aes(label = paste0(n, " (", percent(n/N, accuracy = 1),")")), 
                    position = position_stack(vjust = 0.5))
...
```{r q12_recognition_rationale, results = 'asis'}
...
data %>% select(URN, num_range("Q13_1_",1:5))            %>%
         ...
         distinct(n, N, understanding)                   %>% 
         ggplot(aes(x = understanding, y = n, fill = n)) + 
         geom_col(colour = "black") + 
         scale_fill_distiller(palette = "Blues", direction = 1) +
         geom_text(aes(label = paste0(n, " (", percent(n/N, accuracy = 1),")")), vjust = -0.5, size = 4) +
         theme_bw() + ylab("Numbers") +
...
data %>% select(num_range("Q13_1_",1:5), gender = Q21)     %>%
         ...
         geom_bar(colour = "black") +
         geom_text(aes(label = after_stat(count)), stat = "count",position = position_stack(0.5), size = 4) +
         theme_bw() + ylab("Numbers") +
         xlab("Aware of how s/w I use is funded, managed and licensed") +
         scale_fill_brewer(palette = "Set2", name = "Gender")
```
...
```{r q13_combined}
data %>% select(URN, starts_with("Q13_"))     %>%
         pivot_longer(cols = num_range("Q13_1_",1:5),
                      names_to = NULL,
                      values_to = "Q13a")                                 %>% 
         pivot_longer(cols = num_range("Q13_2_",1:5),
         ...
         pivot_longer(cols = c("Q13a", "Q13b","Q13c","Q13d"),
                      ...
                      values_to ="Response")                                                     %>% 
        filter(Response != 0)                                                                    %>% 
        distinct(URN, Q13, Response)                                             %>% 
        mutate(Response = factor(Response, 
              levels = c("Strongly agree","Agree","Undecided", "Disagree","Strongly disagree"))) %>% 
        ggplot(aes(x = Response, fill = Q13)) + 
        geom_bar(position = position_dodge2(), colour = "black") +
        theme_bw() + ylab("Numbers")
...
data %>% select(URN, starts_with("Q13_"))                     %>%
        ...
        group_by(Q13)                                          %>%
        mutate(N = n())                                        %>% 
        group_by(Q13, Response)                                %>% 
        mutate(p = n()/N)                                      %>% 
        mutate(Response = factor(Response, 
              levels = c("Strongly agree","Agree","Undecided", "Disagree","Strongly disagree"))) %>% 
        distinct(Response, p, Q13) %>% 
        ggplot(aes(x = Response, y = p, fill = Q13)) + 
        scale_y_continuous(labels = percent_format(accuracy = 1)) +
        geom_col(position = position_dodge2(), colour = "black") +
        theme_bw() + ylab("Percent") +
        geom_text(aes(label = percent(p, accuracy = 0.1)), 
                  position = position_dodge2(width = 0.9), vjust = -0.5, size = 4)
...
```{r q13_combined_career}

data %>% select(URN, starts_with("Q13_"), career = Q20)                 %>%
         filter(!(career %in% c("-", "Other")))                         %>% 
         pivot_longer(cols = num_range("Q13_1_",1:5),
                      names_to = NULL,
                      values_to = "Q13a")                               %>% 
         ...
         pivot_longer(cols = c("Q13a", "Q13b","Q13c","Q13d"),
                      ...
                      values_to ="Response")                            %>% 
        filter(Response != 0)                                           %>% 
        distinct(URN, career, Q13, Response)                            %>% 
        group_by(Q13, Response, career)                                 %>% 
        mutate(n = n())                                                 %>% 
        distinct(career, Q13, Response, n)                              %>% 
        mutate(Response = factor(Response, 
              levels = c("Strongly agree","Agree","Undecided", "Disagree","Strongly disagree"))) %>% 
        ggplot(aes(x = Response, y = n, fill = Q13)) + 
        geom_col(position = position_dodge2(), colour = "black") +
        theme_bw() + 
       theme(axis.text.x = element_text(angle = -90, hjust = 0, size = 8)) +
        ylab("Numbers") + facet_wrap(~career, scales = "fixed")
...
data %>%  select(num_range("Q14_", 1:9), career = Q20)  %>%
          ...
          geom_text(aes(label = after_stat(count)), stat = "count", position = position_stack(0.5), size = 2) +
          theme(axis.text.x = element_text(angle = -45, vjust = 0.5, hjust=0)) +
          scale_fill_brewer(palette = "Set1", name = "Career stage")
...
data %>% select(URN, starts_with("Q15_"), -"Q15_a")                     %>%
         ...
         mutate(n = n())                                                %>% 
         mutate(per = percent(n/N, accuracy = 1))                       %>% 
         ggplot(aes(x = reorder(aware, -n), fill = n)) + 
         geom_bar(colour = "black") +
...
data %>% select(URN, starts_with("Q15_"), -"Q15_a", career = Q20)          %>%
         group_by(career)                                                  %>%
         mutate(N = n())                                                   %>% 
         pivot_longer(cols = starts_with("Q15_"),
                      names_to = NULL,
         ...
         mutate(n = n())                                                   %>% 
         mutate(per = percent(n/N, accuracy = 1))                          %>% 
         distinct(n, N, per, career, aware, m)                             %>% 
         ggplot(aes(x = reorder(aware, -m), y = n/N, fill = career)) +
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"), colour = "black") +
         theme_bw() + 
         scale_y_continuous(labels = percent_format(accuracy = 1)) +
         geom_text(aes(label = per), vjust = -0.5,
...
data %>% select(starts_with("Q15_"), -"Q15_a", datause)                %>%
         ...
         mutate(n = n())                                                %>% 
         ggplot(aes(x = reorder(aware, -m), fill = datause)) +
         geom_bar(colour = "black") +
         theme_bw() +
         geom_text(aes(label = after_stat(count)), stat = "count", position = position_stack(0.5), size = 3) +
         ylab("Numbers") + xlab("Aware of the policy for") +
...
data %>% select(URN, starts_with("Q15_"), -"Q15_a", datause)              %>%
         ...
         mutate(n = n())                                                  %>% 
         mutate(per = percent(n/N, accuracy = 1))                         %>% 
         distinct(n, N, per, datause, aware, m)                              %>% 
         ggplot(aes(x = reorder(aware, -m), y = n/N, fill = datause)) +
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"), colour = "black") +
         theme_bw() + 
         scale_y_continuous(labels = percent_format(accuracy = 1)) +
         geom_text(aes(label = per), vjust = -0.5,
...
data %>% select(id = URN, starts_with("Q15_"), -"Q15_a", gender = Q21)    %>%
         ...
         group_by(aware)                                                  %>% 
         mutate(m = n())                                                  %>% 
         distinct(id, aware, gender, N, m)                                %>% 
         filter(!(gender %in% lowstatsgender))                            %>% 
         mutate(aware = factor(aware, ordered = TRUE, levels = licawa))   %>% 
         group_by(gender, aware)                                          %>% 
         mutate(n = n())                                                  %>% 
         mutate(per = percent(n/N, accuracy = 1))                         %>% 
         distinct(n, N, per, gender, aware, m)                            %>% 
         ggplot(aes(x = reorder(aware, -m), y = n/N, fill = gender)) +
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"), colour = "black") +
         theme_bw() + 
         scale_y_continuous(labels = percent_format(accuracy = 1)) +
         geom_text(aes(label = per), vjust = -0.5,
...
data %>% select(id = URN, starts_with("Q15_"), -"Q15_a", career = Q20, 
         ...
         mutate(n = n())                                                  %>% 
         mutate(per = percent(n/N, accuracy = 1))                         %>% 
         distinct(n, N, per, gender, aware, career, m)                    %>% 
         ggplot(aes(x = reorder(aware, -m), y = n/N, fill = gender)) +
         geom_col(position = position_dodge2(width = 0.9, preserve = "single"), colour = "black") +
         theme_bw() + 
...
data %>%  select(URN, num_range("Q16_", 1:13))            %>%
          ...
          group_by(barriers)                              %>%
          mutate(n = n())                                 %>% 
          distinct(n, barriers, N)                        %>% 
          ggplot(aes(x = reorder(barriers, -n), y = n, fill = n)) +
          geom_col(colour = "black") +
          theme_bw() + ylim(0, 91) +
          xlab(my$lab) + ylab("Numbers") + scale_fill_distiller(palette = "Blues", direction = 1) +
          geom_text(aes(label = paste0(n, " (", percent(n/N, accuracy = 1),")")),  vjust = -0.5, size = 4) +
...
funding %>% pivot_longer(cols = num_range("Q18_", 1:7),
            ...
            mutate(n = n())                              %>% 
            filter(n > 2)                                %>%
            distinct(n, funding, N)                      %>% 
            ggplot(aes(x = reorder(funding, -n), y = n, fill = n)) +
            geom_col(colour = "black") + ylim(0, 125) +
            theme_bw() + xlab(my$lab) + ylab("Numbers") +
            scale_fill_distiller(palette = "Blues", direction = 1) +
            geom_text(aes(x = funding, label = paste0(n,"\n(", percent(n/N, accuracy = 1),")")), 
                      vjust = -0.5, size = 3) +
            theme(axis.text.x = element_text(angle = -60, vjust = 0.5, hjust=0, size = 10),
                  legend.position = "None") -> Fig9

# Print to the display
print(Fig9)

# Save to a file
ggsave(filename="Figures/Figure9.png", dpi=360)
...
data %>%  select(URN, num_range("Q19_", 1:22), gender = Q21)           %>%
          ...
          geom_text(aes(x = disciplines, label = after_stat(count)),
...
data %>% select(ethnicity =  Q22)                   %>%
         ...
         ggplot(aes(x = n, y = reorder(ethnicity, -n), fill = n)) +
         geom_col(colour = "black") +
         theme_bw() + 
...
data %>% select(dis = Q23, career = Q20) %>% 
         ...
         geom_text(aes(x = dis, label = after_stat(count)),
